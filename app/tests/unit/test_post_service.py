...
async def test_update_post():
    ...
    with patch("app.services.posts.PostRepository.is_exists", new=AsyncMock(return_value=True)) as mock_is_exists, \
        patch("app.services.posts.PostRepository.get_by_id", new=AsyncMock(return_value=mock_post_to_update)) as mock_get_by_id, \
        ...
        mock_is_exists.assert_called_once_with(mock_db, post_id)
...
async def test_update_non_existent_post():
    ...
    with patch("app.services.posts.PostRepository.is_exists", new=AsyncMock(return_value=False)) as mock_is_exists:
